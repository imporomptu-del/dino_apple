...
Pipeline order per frame:
  ...
  4. Grayscale
  5. MOG2 background subtraction  ← replaces absdiff; learns static background
...
# NvOF scale is set via --nvof-scale CLI argument (default 0.5, use 1.0 for full res)
...
class HybridFlowEstimator:
    ...
    def _init_nvof(self) -> None:
        nvof_w = max(8, int(self.full_w * self.nvof_scale) // 4 * 4)
        nvof_h = max(8, int(self.full_h * self.nvof_scale) // 4 * 4)
        try:
            ...
            print(f"Flow backend: NVIDIA OFA (NvidiaOpticalFlow_2_0) "
                  f"at {nvof_w}x{nvof_h} ({self.nvof_scale:.0%} of {self.full_w}x{self.full_h})")
...
def process_video(
    ...
    print(f"Background subtractor: MOG2 "
          f"(history={mog2_history} frames, threshold={mog2_threshold})")
    print(f"NvOF scale: {nvof_scale} "
          f"({'full resolution' if nvof_scale >= 1.0 else f'{int(nvof_scale*100)}% of frame'})")
    ...
    try:
        with open(output_csv, "w", newline="") as f:
            ...
            while True:
                ...
                # ── STEP 4: MOG2 background subtraction ───────────────────
                ...
                if process_scale != 1.0:
                    ...
                    fg_mask = cv2.resize(
                        fg_mask, (width, height), interpolation=cv2.INTER_NEAREST)
                ...
                # fg_mask OR flow — detect if either condition is true
                motion_mask = cv2.bitwise_or(fg_mask, flow_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mog2-history",     type=int,   default=500,
        help="Frames MOG2 uses to learn background (default 500 ≈ 15s at 32fps)")
    parser.add_argument("--mog2-threshold",    type=float, default=16.0,
        help="MOG2 sensitivity — lower catches subtler movement (default 16)")
    parser.add_argument("--nvof-scale",        type=float, default=0.5,
        help="NvOF internal resolution scale (default 0.5, use 1.0 for full res)")
    ...
    process_video(
        ...
        mog2_threshold = args.mog2_threshold,
        nvof_scale     = args.nvof_scale,
